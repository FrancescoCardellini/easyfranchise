...
public class Util {
    ...
    public static String getDBAdminServiceUrl(String tenantId) {
        Properties p = readProperties(BACKEND_CONFIG_PATH);
        return p.getProperty(DB_ADMIN_SERVICE);
    }

    public static String getDBSqlEndpoint(String tenantId) {
    	String path = DB_SECRET_PATH + "-" + tenantId;
        Properties p = readProperties(path);
        return p.getProperty(DB_SQLENDPOINT);
    }

    public static String getDBAdmin(String tenantId) {
    	String path = DB_SECRET_PATH + "-" + tenantId;
        Properties p = readProperties(path);
        return p.getProperty(DB_ADMIN);
    }

    public static String getDBPassword(String tenantId) {
    	String path = DB_SECRET_PATH + "-" + tenantId;
        Properties p = readProperties(path);
