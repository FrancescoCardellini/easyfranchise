...
public class DB {
	 ...
	 * Then all connections go via MutliTenantProvider. If this connection does not
	 * succeed all info is deleted and reset for next connection.
	 * 
	 * @param tenantId
	 * @param persistenceUnit
	 * @return
	 */
	private static EntityManagerFactory getEntityManagerFactory(String dbschema, String persistenceUnit,
			String tenantId) {
...
//                logger.info("initiate EntityManagerFactory for persistence unit " + persistenceUnit);
				...
				emf = null;
				logger.error("Could not initiate EntityManagerFactory for" + dbschema + ":", e);
			}
		}
		return emf;
	}
...
//        dbschema = dbschema.toUpperCase();
		...
		final SessionFactoryImplementor sessionFactory = ((org.hibernate.internal.SessionFactoryImpl) emf)
				.getSessionFactory();
...
//        schemaResolver.setTenantIdentifier(dbschema);
	...
	/**
	 * Read Password for schema user. Reads all entries from admin-only table Tenant
	 * and returns password if schema was found.
	 * 
	 * @param schemaName
	 * @return
	 */
...
//        if (Util.getDBAdmin(tenantId).equalsIgnoreCase(schemaName)) {
		...
		try {
			...
			// if there is an exception it will occur during DB connection creation in
			// hibernate threads
	...
	/**
	 * Build JDBC connection URL. Package visibility to allow access for DB classes
	 * in same package.
	 * 
	 * @return
	 */
	static String getConnectionUrl(String tenantId) {
		return "jdbc:sap://" + Util.getDBSqlEndpoint(tenantId) + "/?encrypt=true";
	}
	 ...
	 * @return
	 */
	public static List<Tenant> getAllTenants(String tenantId) {
		EntityManager em = getEntityManager(Util.getDBAdmin(tenantId), tenantId);
		try {
			logger.info("got EntityManager " + em);
	...
	/**
	 * Onboard subaccount. Fails if this tenant id was already saved or schema/user
	 * name already used. Will create new user and schema, then create all needed
	 * tables and finally create CONFIGURATION entry with default data.
	 * 
	 * @param tenantId
	...
	public static String onboard(String tenantId, TenantInfo tenantInfo) {
		...
		String db_sqlEndoint = Util.getDBSqlEndpoint(tenantId);
		String db_admin = Util.getDBAdmin(tenantId);
		String db_password = Util.getDBPassword(tenantId);
		if (db_sqlEndoint == null || db_admin == null || db_password == null) {
			String msg = "DB configuration error: sqlEndpoint " + db_sqlEndoint + " admin user " + db_admin;
			if (db_password == null) {
				msg += " password null";
			}
		...
		logger.warn("onboard with tenant " + tenantId + " schema " + schema + " and subdomain " + tenantInfo.subdomain
				+ " DB admin " + Util.getDBAdmin(tenantId));

		// DB admin creates DB user and schema for the new tenant:
		EntityManager emAdmin = getEntityManager(Util.getDBAdmin(tenantId), tenantId);
		try {
...
//            q.executeUpdate();
			...
			c.setLogoUrl(
					"https://raw.githubusercontent.com/SAP-samples/btp-kyma-multitenant-extension/main/code/easyfranchise/source/ui/src/assets/logo-placeholder.png");
			emAdmin.merge(c);
...
//		EntityManager emTenant = getEntityManager(schema.toUpperCase(), tenantId);
//		try {
...
//		}
	...
	private static void validateUniquenesOfSubdomain(EntityManager emAdmin, String subdomain) {
		TypedQuery<Tenant> query = emAdmin.createQuery("SELECT t FROM Tenant t WHERE t.subdomain = :subdomain",
				Tenant.class);
	...
	public static String offboard(String tenantId) {
		String db_sqlEndpoint = Util.getDBSqlEndpoint(tenantId);
		String db_admin = Util.getDBAdmin(tenantId);
		String db_password = Util.getDBPassword(tenantId);
		...
		logger.warn("offboard tenant " + tenantId + " DB admin " + Util.getDBAdmin(tenantId));

		// The admin database user remove this tenant from tenant table and delete
		// tenant schema:
		EntityManager emAdmin = getEntityManager(Util.getDBAdmin(tenantId), tenantId);
		try {
...
//            q.executeUpdate();
	...
	public static Configuration getConfig(String tenantId) {
		...
		EntityManager em = getEntityManager(dbschema, tenantId);
	...
	public static Coordinator createOrUpdateCoordinator(String tenantId, Coordinator coordinator) {
		...
		try {
			...
			if (coordinator.getId() != 0L) {
				...
				if (old == null) {
					// old entity dose not exist
					throw new WebApplicationException("Could not find coordinator for update " + coordinator.getId(),
							550);
				}
			}
