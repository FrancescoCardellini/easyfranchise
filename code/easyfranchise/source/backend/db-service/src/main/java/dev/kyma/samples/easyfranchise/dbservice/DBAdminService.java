...
public class DBAdminService extends BaseRS {
    ...
    @GET
    @Path("config/tenants")
    public Response getTenants(@PathParam("tenantId") String tenantId, @Context ContainerRequestContext resContext) { // XXX aggiunto tenantId
        logger.info(Util.createLogDetails(resContext));
        try {
            List<Tenant> allTenants = DB.getAllTenants(tenantId);
