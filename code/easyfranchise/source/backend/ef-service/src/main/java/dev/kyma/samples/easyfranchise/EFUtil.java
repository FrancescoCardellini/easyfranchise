...
public class EFUtil {
     ...
     */
    public static void sendMailToCoordinatorForMentorsWithoutFranchisee(String tenantId) {
            ...
            try {
                ...
                if (newFranchiseNames.size() > 0) {
                    ...
                    if (coordinators.size() > 0) {
                        ...
                        ConnectionParameter param = new ConnectionParameter(RequestMethod.PUT, Util.getEmailServiceUrl(tenant.getTenantid()) + "notifycoordinator");
     ...
     * @return
     */
    private static List<Tenant> getTenants(String tenantId) {
        ConnectionParameter param = new ConnectionParameter(Util.getDBAdminServiceUrl(tenantId) + "config/tenants").setAcceptJsonHeader();
        Connection.call(param);
        if (param.status != HttpStatus.SC_OK) { 
